...
public class BresenhamLineDrawerParagraph implements LineDrawer {

    private PixelDrawer pd;
    private Color c;

    public BresenhamLineDrawerParagraph(PixelDrawer pd) {
        this.pd = pd;
        this.c = Color.BLACK;
    }
    ...
    public void drawLine(ScreenPoint p1, ScreenPoint p2) {
        ...
        if (lengthY <= lengthX)
        {
            ...
            while(length != 0)
            {
                ...
                pixelCount = pixelCount == 12 ? 0:pixelCount;
                pixelCount++;
                x += dx;
                d += 2 * lengthY;
                if (d > 0) {
